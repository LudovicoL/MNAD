...
def CreateAitexDataset(AitexFolder):
    ...
    train_range = [0, len(nodefect_images) - int(len(nodefect_images) * 10 / 100)]  # 60% train set without defects
    validation_range_without_defect = [len(nodefect_images) - int(len(nodefect_images) * 10 / 100),
                                 len(nodefect_images)]  # 10% train set without defects
    validation_range_with_defect = [0, len(defect_images) - int(
        len(defect_images) * 80 / 100)]  # 20% validation set with defects
    test_range_with_defect = [len(defect_images) - int(len(defect_images) * 80 / 100),
                              len(defect_images)]  # 80% validation set with defects
...
def checkAitex():
    if os.path.isdir(aitex_folder):
        if (os.path.isdir(aitex_train_dir) and os.path.isdir(aitex_validation_dir) and os.path.isdir(aitex_test_dir) and os.path.isdir(aitex_mask_dir)):
            ...
            print("Preparing the AITEX dataset...")
